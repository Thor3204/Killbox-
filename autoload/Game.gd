...
func set_player_name(new_name: String) -> void:
	...
	if player_name.is_empty(): player_name = "Superviviente"
	_save_profile()

func complete_profile(character_id: String) -> void:
	selected_character_id = character_id
	profile_completed = true
	if box <= 0.0: box = TEST_STARTING_BOX
	_save_profile()

func select_character(character_id: String) -> bool:
	if character_id == selected_character_id: return true
	if box < CHARACTER_CHANGE_COST: return false
...
func add_stars(amount: int) -> void:
	stars += amount; stars_changed.emit(stars); _save_profile()

func add_box(amount: float) -> void:
	box += amount; box_changed.emit(box); _save_profile()

func _save_profile() -> void:
	var data := {"player_name":player_name,"selected_character_id":selected_character_id,"stars":stars,"box":box,"profile_completed":profile_completed}
...
func _load_profile() -> void:
	...
	# Profiles created by the old prototype did not have the V1 test balance.
	if not profile_completed and box <= 0.0: box=TEST_STARTING_BOX
