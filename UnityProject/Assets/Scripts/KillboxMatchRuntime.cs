...
public class KillboxMatchRuntime : MonoBehaviour
{
    ...
    void SpawnBot(){
        int count=0; foreach(var g in GameObject.FindObjectsOfType<GameObject>()) if(g.name.StartsWith("Bot")) count++;
        if(count>=12)return;
        var b=GameObject.CreatePrimitive(PrimitiveType.Capsule); b.name="Bot_"+count; b.transform.position=new Vector3(Random.Range(-30,30),1,Random.Range(-30,30)); b.GetComponent<Renderer>().material.color=new Color(.95f,.25f,.25f);
    }
}
